...
from gaia.esgf.openssl import OpenSSLConfig
...
so that any error set upstream overrides and is honoured.
    ...
    def get_verify_server_cert_func(self):
        def verify_server_cert(connection,
                               peerCert,
                               errorStatus,
                               errorDepth,
                               preverifyOK):
...
TRUSTED_CERTS=1"""
    ...
    PROXY_FILE_PERMISSIONS = int('600', 8)
    ...
    DEF_PROXY_FILEPATH = sys.platform == (
        'win32' and 'proxy' or
        sys.platform in ('linux2', 'darwin') and
        '/tmp/x509up_u%s' % (os.getuid()) or None)
...
If creds already, exist exit without proceeding
        ...
        if six.PY2 and isinstance(username, six.text_type):
            username = str(username)

        if six.PY2 and isinstance(passphrase, six.text_type):
...
private key is not password protected.
        ...
        if credname:
            if not isinstance(credname, six.string_types):
